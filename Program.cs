...
class ChangelogGenerator
{
    ...
    static async Task Run(Options options)
    {
        ...
        var aiChangelogBuilder = new System.Text.StringBuilder();
        foreach (var prNumber in prNumbers)
        {
            var details = await fetchPullRequestContext(client, owner, repo, prNumber);
            var summary = await SummarizeWithLLM(httpClient, details);

            aiChangelogBuilder.AppendLine(summary);
        }
        
        Console.WriteLine(aiChangelogBuilder.ToString());
    }
    ...
    static async Task<string> SummarizeWithLLM(HttpClient httpClient, string context)
    {
        int maxRetries = 3;
        int retryCount = 0;
